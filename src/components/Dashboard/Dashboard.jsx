    ...
    fetch(`${process.env.REACT_APP_BASE_URL}lab-status/`)
      ...
      .then((data) => {
        ...
        if (today) {
          openTime =
            "Today, " +
            openTime.toLocaleTimeString("gr", { hour12: false }).slice(0, 5);
        } else {
          openTime =
    ...
    <div className="dashboard">
      <div className="header">
        ...
        {labStatus.closed == "Open " && (
          <h2 className="lab-status">Lab is {labStatus.closed} &#128275;</h2>
        )}
        {labStatus.closed == "Closed " && (
          <h2 className="lab-status">Lab is {labStatus.closed} U+1F512</h2>
        )}
      </div>
      <div className="cards-container">
        <StatisticsCard
          info={`${Math.floor(labStatus.total_hours / 3600)} h and ${Math.floor(
            (labStatus.total_hours % 3600) / 60
          )} min`}
        />
        <StatisticsCard title="Vroomers at Lab" info={usersInLab} />
        <StatisticsCard title="Lab Opened" info={labStatus.opened_time} />
